...
<script>
  <?php
  foreach ($grafik_perempuan as $key => $value) {
    $status_gizi_perempuan[] = $value->status_gizi;
    $total_perempuan[] = $value->jml;
  }
  ?>
  var ctx = document.getElementById('grafik_perempuan');
  var grafik = new Chart(ctx, {
    type: 'bar',
    data: {
      labels: <?= json_encode($status_gizi_perempuan) ?>,
      datasets: [{
        label: 'Grafik Jumlah Status Gizi Perempuan Perbulan',
        data: <?= json_encode($total_perempuan) ?>,
        backgroundColor: [
          'rgba(255, 99, 132, 0.80)',
          'rgba(54, 162, 235, 0.80)',
          'rgba(255, 206, 86, 0.80)',
...
<script>
  <?php
  foreach ($grafik_laki as $key => $value) {
    $status_gizi_laki[] = $value->status_gizi;
    $total_laki[] = $value->jml;
  }
  ?>
  var ctx = document.getElementById('grafik_laki');
  var grafik = new Chart(ctx, {
    type: 'bar',
    data: {
      labels: <?= json_encode($status_gizi_laki) ?>,
      datasets: [{
        label: 'Grafik Jumlah Status Gizi Laki - Laki Perbulan',
        data: <?= json_encode($total_laki) ?>,
        backgroundColor: [
          'rgba(255, 99, 132, 0.80)',
          'rgba(54, 162, 235, 0.80)',
          'rgba(255, 206, 86, 0.80)',
