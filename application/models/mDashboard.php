...
class mDashboard extends CI_Model
{
    ...
    //dashboard laki-laki
    public function grafik_perempuan($bulan)
    {
        return $this->db->query("SELECT COUNT(status_gizi) as jml, status_gizi, jenis_kelamin FROM balita JOIN lap_status_gizi ON balita.id_balita=lap_status_gizi.id_balita WHERE jenis_kelamin='P' AND MONTH(tgl_periksa)='" . $bulan . "' GROUP BY status_gizi ORDER BY jml DESC")->result();
    }
    public function grafik_laki($bulan)
    {
        return $this->db->query("SELECT COUNT(status_gizi) as jml, status_gizi, jenis_kelamin FROM balita JOIN lap_status_gizi ON balita.id_balita=lap_status_gizi.id_balita WHERE jenis_kelamin='L' AND MONTH(tgl_periksa)='" . $bulan . "' GROUP BY status_gizi ORDER BY jml DESC")->result();
    }
    ...
    public function status_gizi($bulan, $tahun)
    {
        $this->db->select('*');
        $this->db->from('lap_status_gizi');
        $this->db->join('balita', 'lap_status_gizi.id_balita = balita.id_balita', 'left');
